<template>
        <span class="badge bg-danger" v-if="viewsNumber"> {{viewsNumber}} <i class="far fa-eye"></i></span>
</template>
        ...
        methods: {
          ...
          //  ...mapActions(['fetchPosts']),
            ...
            addView(){
                ...
                this.$store.dispatch('updateView', articleId)
                // удаление коммента из массива по ключу id
            },
        },
        computed: {
         ...
         /*   state () {
            ...
            viewsNumber(){
                let number = this.$store.getters.getterArticleViews
                return number
            },
